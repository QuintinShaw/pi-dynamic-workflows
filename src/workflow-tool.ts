...
const workflowToolSchema = Type.Object({
  ...
  tokenBudget: Type.Optional(
    Type.Number({
      description:
        "Optional cumulative-session-token launch gate for the run. It is checked before each agent starts and charged after completion, so agents already running in parallel can overshoot it; later launches then fail. Usage includes input, output, cache-read, and cache-write tokens across every assistant turn. Omit for no gate. Set it only when the user explicitly asks to cap tokens/spend.",
    }),
  ),
});
  ...
  return defineTool({
    ...
    get promptGuidelines() {
      return [
        ...
        "For workflow, tokenBudget is a cumulative-session-token launch gate, not a hard in-flight cap: usage is charged after each agent, includes input/output/cache-read/cache-write across every assistant turn, and a concurrent wave can overshoot before later launches are blocked. Per-phase phase('Name', {budget: N}) uses the same between-launch semantics — wrap it in try/catch so later phases proceed. Use retry(thunk, {attempts, until}) for bounded retry, gate(thunk, validator, {attempts}) for feedback-driven attempts, and budget.remaining() to skip optional rounds or choose a lighter tier.",
