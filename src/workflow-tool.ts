  ...
  return defineTool({
    ...
    get promptGuidelines() {
      return [
        ...
        "For workflow, prefer the built-in quality helpers when they fit (each is built on agent()/parallel() and returns plain data): verify(item, {reviewers, threshold, lens, tier, model, agentType}) for adversarial fact-checking; judgePanel(attempts, {judges, rubric, tier, model, agentType}) to score N candidates and return the best; loopUntilDry({round, key, consecutiveEmpty}) to keep finding until rounds stop yielding new items; completenessCheck(args, results, {tier, model, agentType}) as a final 'what's missing' critic. Route helper-created agents explicitly just like direct agent() calls.",
